...
## Credits to: https://github.com/wimpysworld/nix-config/blob/main/home-manager/_mixins/desktop/hyprland/fuzzel/fuzzel-hyprshot.sh
...
NOTIFY="notify-send --app-name=$APP_NAME"
...
"󰩭 Screenshot a Region")
  hyprshot --mode region --raw --silent | satty --filename - --output-filename ~/Pictures/Screenshots/satty-"$(date '+%Y%m%d-%H:%M:%S')".png
  ;;
"󰖯 Screenshot a Window")
